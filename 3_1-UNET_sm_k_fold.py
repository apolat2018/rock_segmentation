...
class Dataset:
    ...
    def __init__(
        ...
        if pairs is not None:
            self.ids = [os.path.basename(p[0]) for p in pairs]
...
EPOCHS = 100
#Training with sklearn KFold cross-validation
cc=3
...
train_ids_all = os.listdir(x_train_dir)
valid_ids_all = os.listdir(x_valid_dir)
all_pairs = [(os.path.join(x_train_dir, img_id), os.path.join(y_train_dir, img_id)) for img_id in train_ids_all]
all_pairs += [(os.path.join(x_valid_dir, img_id), os.path.join(y_valid_dir, img_id)) for img_id in valid_ids_all]
all_pairs = np.array(all_pairs, dtype=object)
fold_results = []
print(f"Train klasoru ornek sayisi: {len(train_ids_all)}")
print(f"Valid klasoru ornek sayisi: {len(valid_ids_all)}")
print(f"Cross validation toplam ornek sayisi: {len(all_pairs)}")

kf = KFold(n_splits=cc, shuffle=True, random_state=42)
for i, (train_idx, valid_idx) in enumerate(kf.split(all_pairs)):
  valid_pairs = all_pairs[valid_idx].tolist()
  train_pairs = all_pairs[train_idx].tolist()
  print(f"Fold {i+1}/{cc} -> Train ornek: {len(train_pairs)}, Valid ornek: {len(valid_pairs)}")
...
# compile keras model with defined optimozer, loss and metrics
  ...
  fold_results.append({
      ...
      "val_iou": scores[1],
      "val_f1": scores[2],
      "train_iou": history.history['iou_score'][-1],
      "train_f1": history.history['f1-score'][-1],
  })
  print(f"Fold {i+1}/{cc} -> Val IoU: {scores[1]:.6f}, Val F1: {scores[2]:.6f}, Train IoU: {history.history['iou_score'][-1]:.6f}, Train F1: {history.history['f1-score'][-1]:.6f}")
#--------------------save as txt------------
...
if len(fold_results) > 0:
  mean_val_iou = np.mean([x["val_iou"] for x in fold_results])
  std_val_iou = np.std([x["val_iou"] for x in fold_results])
  mean_val_f1 = np.mean([x["val_f1"] for x in fold_results])
  std_val_f1 = np.std([x["val_f1"] for x in fold_results])

  print("============= 5-FOLD CROSS VALIDATION SUMMARY =============")
  for r in fold_results:
    print(f"Fold {r['fold']}: Val IoU={r['val_iou']:.6f}, Val F1={r['val_f1']:.6f}, Train IoU={r['train_iou']:.6f}, Train F1={r['train_f1']:.6f}")
  print(f"Mean Val IoU: {mean_val_iou:.6f} +- {std_val_iou:.6f}")
  print(f"Mean Val F1 : {mean_val_f1:.6f} +- {std_val_f1:.6f}")

  f=open(os.path.join(save_path,"rslts.txt"),"a+")
  f.write("============= 5-FOLD CROSS VALIDATION SUMMARY =============\n")
  for r in fold_results:
    f.write(f"Fold {r['fold']}: Val IoU={r['val_iou']:.6f}, Val F1={r['val_f1']:.6f}, Train IoU={r['train_iou']:.6f}, Train F1={r['train_f1']:.6f}\n")
  f.write(f"Mean Val IoU: {mean_val_iou:.6f} +- {std_val_iou:.6f}\n")
  f.write(f"Mean Val F1 : {mean_val_f1:.6f} +- {std_val_f1:.6f}\n")
  f.write("============================================================\n")
  f.close()
